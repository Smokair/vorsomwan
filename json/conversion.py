...
for v5 in v1["rack"]:
    print('------5A--------')
    print(type(v5))
    #print(v5)### JSON CONVERSION EXERCISE ###
...
# Step 1: Original data type
print('------1---------')
print(f"Type of dc_container: {type(dc_container)}")

# Step 2: Convert JSON string to dictionary
v1 = json.loads(dc_container)  # loads: str => dict
print('------2---------')
print(f"Type of v1: {type(v1)}")

# Step 3: Convert dictionary back to JSON string
v2 = json.dumps(v1, indent=4)  # dumps: dict => str
print('------3---------')
print(f"Type of v2: {type(v2)}")

# Step 4: Convert dictionary to YAML format
v3 = yaml.dump(v1)  # dump: dict => YAML
print('------4---------')
print("YAML representation of v1:")
print(v3)

# Step 5: Extract specific elements
print('------5---------')
v4 = v1["rack"][0]
print(f"Type of v4: {type(v4)}")
print("Keys in the first service of the first server:")
print(v4["server"]["services"][0].keys())

# Step 6: Loop through servers and their services
print('------5---------')
for server_data in v1["rack"]:
    print('------5A--------')
    print(f"Type of server_data: {type(server_data)}")
    print(f"Operating system: {server_data['server']['os']}")
    for service in server_data["server"]["services"]:
        print(f"Service: {service['service']}, Port: {service['port']}")

# Step 7: Display keys of the first service in the last server
print('------5---------')
print("Keys in the first service of the last server:")
print(v1["rack"][-1]["server"]["services"][0].keys())
